...
static int ini_brew_readline(IFile* file,char* buffer,int size);
 ...
 */
int ini_gets(const TCHAR *Section, const TCHAR *Key,
             TCHAR *Buffer, int BufferSize, const TCHAR *Filename,void* FileMgr)
{
  INI_FILETYPE fp;
  int ok = 0;
 ...
 */
int ini_getl(const TCHAR *Section, const TCHAR *Key, long* val, const TCHAR *Filename,void* FileMgr) {
  TCHAR LocalBuffer[64];
  int len = ini_gets(Section, Key,LocalBuffer, sizearray(LocalBuffer), Filename,FileMgr);

  if( len > 0 ) {
	  if( len >= 2 && (LocalBuffer[1] == 'x' || LocalBuffer[1] == 'X') ) {
		  *val =  _tcstol(LocalBuffer, NULL, 16);
	  } else {
		  *val = _tcstol(LocalBuffer, NULL, 10);
	  }
  }
  return len;
}
 ...
 * \return            the value located at Key
 */
INI_REAL ini_getf(const TCHAR *Section, const TCHAR *Key, INI_REAL* val, const TCHAR *Filename,void* FileMgr)
{
  TCHAR LocalBuffer[64];
  int len = ini_gets(Section, Key, LocalBuffer, sizearray(LocalBuffer), Filename,FileMgr);
  if( len > 0 ) {
	  *val = ini_atof(LocalBuffer);
	  return 1;
  } else {
	  return 0;
  }
 ...
 */
int ini_getbool(const TCHAR *Section, const TCHAR *Key, int* val, const TCHAR *Filename,void* FileMgr)
{
  TCHAR LocalBuffer[2];
  int ret;

  ret = 0;
  if( ini_gets(Section, Key,LocalBuffer, sizearray(LocalBuffer), Filename,FileMgr) > 0 ) {
 ...
 * \return            the number of characters copied into the supplied buffer
 */
int  ini_getsection(int idx, TCHAR *Buffer, int BufferSize, const TCHAR *Filename,void* FileMgr)
{
  INI_FILETYPE fp;
  int ok = 0;
 ...
 * \return            the number of characters copied into the supplied buffer
 */
int  ini_getkey(const TCHAR *Section, int idx, TCHAR *Buffer, int BufferSize, const TCHAR *Filename,void* FileMgr)
{
  INI_FILETYPE fp;
  int ok = 0;
 ...
 * \param FileMgr    arbitrary data, which the function passes on the the
 *                    \c Callback function
 * \param Filename    the name and full path of the .ini file to read from
 *
...
int  ini_browse(INI_CALLBACK Callback, const mTCHAR *Filename,void *FileMgr,void* pUser)
{
  ...
  for ( ;; ) {
    ...
    *ep++ = '\0';             /* split the key from the value */
    ...
    /* call the callback */
    if (!Callback(LocalBuffer, LocalBuffer + lenSec, LocalBuffer + lenSec + lenKey, pUser))
      break;
  } /* for */
...
static int close_rename(INI_FILETYPE *rfp, INI_FILETYPE *wfp, const TCHAR *filename, TCHAR *buffer,void* FileMgr)
{
  (void)ini_close(rfp);
  (void)ini_close(wfp);
  (void)ini_remove(filename,FileMgr);
  (void)ini_tempname(buffer, filename, INI_BUFFERSIZE);
  (void)ini_rename(buffer, filename,FileMgr);
  return 1;
}
 ...
 */
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value, const TCHAR *Filename,void* FileMgr)
{
  INI_FILETYPE rfp;
  INI_FILETYPE wfp;
  INI_FILEPOS mark;
  ...
  if (len > 0) {
    do {
      if (!ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp)) {
        ...
        return close_rename(&rfp, &wfp, Filename, LocalBuffer,FileMgr);  /* clean up and rename */
      } /* if */
 ...
 * \return            1 if successful, otherwise 0
 */
int ini_putl(const TCHAR *Section, const TCHAR *Key, long Value, const TCHAR *Filename,void* FileMgr)
{
  TCHAR LocalBuffer[32];
  long2str(Value, LocalBuffer);
  return ini_puts(Section, Key, LocalBuffer, Filename,FileMgr);
}
 ...
 * \return            1 if successful, otherwise 0
 */
int ini_putf(const TCHAR *Section, const TCHAR *Key, INI_REAL Value, const TCHAR *Filename,void* FileMgr)
{
  TCHAR LocalBuffer[64];
...
static int ini_brew_readline(IFile* file,char* buffer,int size) {
	...
	for( len = 0; len < size - 1; ++len ) {
		if( IFILE_Read(file,&buffer[len],1) <= 0 ) {
			buffer[len] = 0;
			break;
		}
